...
func Run(completeOptions completedServerRunOptions, stopCh <-chan struct{}) error {
	...
	// 创建并启动server
	/**
		创建并启动server，主要处理kubeAPIServer和apiExtensionsServer
		kubeAPIServer：是kubernetes独有的API Server，它负责的是kubernetes最主要的api，官方文档里面不建议在这个里面添加和修改api
		apiExtensionsServer：定义可扩展的API
	 */
...
func CreateKubeAPIServer(kubeAPIServerConfig *master.Config, delegateAPIServer genericapiserver.DelegationTarget, admissionPostStartHook genericapiserver.PostStartHookFunc) (*master.Master, error) {
	/**
		kubeAPIServerConfig是创建server所用到资源（resource）和配置，complete方法自动填充那些不能为空的字段值，
		new方法根据传入的config返回一个Server的实例。
	 */
